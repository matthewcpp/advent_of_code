...
Kind determine_kind(const std::string& cards) {
    ...
    if (card_counts.size() <= 1) {
        return FiveOfAKind;
    } else if (card_counts.size() == 2 && card_counts[0].second + joker_count == 4) {
        return FourOfAKind;
    } else if (card_counts.size() == 2 && card_counts[0].second + joker_count == 3) {
        return FullHouse;
    } else if (card_counts.size() == 3 && card_counts[0].second + joker_count == 3) {
        return ThreeOfAKind;
    } else if (card_counts.size() == 3 && card_counts[0].second == 2 && card_counts[1].second == 2) {
        return TwoPair;
    } else if (card_counts.size() == 4 && card_counts[0].second + joker_count == 2) {
        return OnePair;
    } else {
        return HighCard;
    }
